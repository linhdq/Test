...
jlongArray Java_com_googlecode_tesseract_android_OCR_combineSelectedPixa(JNIEnv *env, jobject thiz,
                                                                         jlong nativePixaText,
                                                                         jlong nativePixaImage,
                                                                         jintArray selectedTexts,
                                                                         jintArray selectedImages) {
    LOGV(__FUNCTION__);
    ...
    combineSelectedPixa(pixaTexts, pixaImages, textindexes, textCount, imageindexes, imageCount,
                        messageJavaCallback, &pixFinal, &pixOcr, &boxaColumns, true);
...
jint Java_com_googlecode_tesseract_android_OCR_nativeAnalyseLayout(JNIEnv *env, jobject thiz,
                                                                   jint nativePix) {
    LOGV(__FUNCTION__);
...
JNIEXPORT jobject
Java_com_renard_ocr_cropimage_processing_Blur_nativeBlurDetect(JNIEnv *env, jobject thiz,
                                                               jlong nativePix) {
    Pix *pixOrg = (PIX *) nativePix;
    ...
    log("pix = %p, blur=%f, box=(%i,%i - %i,%i)processing time %f\n", pixBlended, blurValue, x, y,
        w, h, stopTimerNested(timer));
    //create result
    jclass cls = env->FindClass("com/renard/ocr/cropimage/processing/BlurDetectionResult");
    jmethodID constructor = env->GetMethodID(cls, "<init>", "(JDJ)V");
    return (*env)->NewObject(cls, constructor, (jlong) pixBlended, (jdouble) blurValue,
                             (jlong) maxBlurLoc);
}

JNIEXPORT jstring  Java_com_renard_ocr_cropimage_processing_Blur_getString
        (JNIEnv* env, jobject obj){
    return (*env)->NewStringUTF(env, "Hello World!");
}


jlong Java_com_googlecode_tesseract_android_OCR_nativeOCRBook(JNIEnv *env, jobject thiz,
                                                              jlong nativePix) {
    LOGV(__FUNCTION__);
    Pix *pixOrg = (PIX *) nativePix;
    Pix *pixText;
